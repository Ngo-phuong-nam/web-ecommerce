    ...
    <div class="container-fluid">
        <div class="card shadow mb-4 p-4">
            ...
            @foreach ($brand as $key => $item)
                <form method="POST" role="form" action="{{ URL::to('admin/update-brand/' . $item->brand_id) }}">
                    {{ csrf_field() }}
                    @method('PUT')
                    <div class="form-group">
                        <label for="exampleFormControlInput1" class="p-2">Tên danh mục</label>
                        <input type="text" name="brand_name" value="{{ $item->brand_name }}"
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="brand_desc" class="form-control" id="exampleFormControlTextarea1"
                            rows="4">{{ $item->brand_desc }}</textarea>
                    </div>
                    <hr/>                   
